...
class CheckoutController extends Controller
{
    ...
    public function confirm_order(Request $request){
        ...
        if(Session::get('cart')==true){
           foreach(Session::get('cart') as $key => $cart){
               ...
               $orderDetail->product_coupon =  $data['coupon_value'];
               $orderDetail->product_feeship = $data['fee_shipping'];
               $orderDetail->save();
           }
        }
    ...
    public function checkout(Request $request) {
        ...
        $meta_keywords = "giao hàng Handmade, Handmade checkout";
        $meta_canonical = $request->url();
        $image_og = "";
    ...
    public function delete_order($order_code) {
        // Xác thực quyền truy cập (nếu cần)
        $this->AuthLogin();
        
        // Kiểm tra nếu không tìm thấy đơn hàng với order_code
        ...
        if (!$order) {
            ...
            Session::put('message', 'Không tìm thấy đơn hàng cần xóa');
            return Redirect::to('/manage-order');
        }
    
        // Xóa các chi tiết đơn hàng liên quan trong tbl_order_details
        DB::table('tbl_order_details')->where('order_code', $order_code)->delete();
    
        // Xóa đơn hàng trong tbl_order
