...
class ProductController extends Controller
{
    ...
    public function detail_product($product_id, Request $request) {
        ...
        $branch_product = DB::table('tbl_branch_product')
            ->where('branch_status', '1')
            ->orderby('branch_id', 'desc')
            ->get();
    
        $product_by_id = DB::table('tbl_product')
            ->join('tbl_branch_product', 'tbl_product.branch_id', '=', 'tbl_branch_product.branch_id')
            ->join('tbl_category_product', 'tbl_product.category_id', '=', 'tbl_category_product.category_id')
        ...
        if ($category_id !== null) {
            ...
            $relate_product = collect([]); // Danh sách rỗng nếu không có category_id
        }
    
        $meta_title = $meta_desc = $meta_keywords = $meta_canonical = $image_og = null;
